...
$compra=mysql_query("SELECT compra.id_compra, compra.fecha, compra.id_proveedor, compra.id_usuario, compra_detalle.cod_producto, compra_detalle.cantidad, compra_detalle.costo, productos.nombre, proveedores.nombre, usuarios.usuario FROM compra, compra_detalle, productos, proveedores, usuarios WHERE compra.id_compra=compra_detalle.id_compra AND compra_detalle.cod_producto=productos.cod AND proveedores.id_proveedor=compra.id_proveedor AND compra.id_usuario=usuarios.id_usuario ORDER BY compra.fecha DESC",$link);
if(isset($_GET['exp']))
{
...
<?
while($compr=mysql_fetch_array($compra))
{
?>
  <tr> 
