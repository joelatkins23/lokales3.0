...
$ventas=mysql_query("SELECT ventas.id_venta, ventas.id_usuario, ventas.fecha, ventas.hora, ventas.id_forma, ventas_detalle.id_venta, ventas_detalle.cod_producto, ventas_detalle.cantidad, ventas_detalle.precio, productos.nombre, usuarios.usuario, ventas_forma_pago.nombre, ventas.efectivo, ventas.descuento, ventas.recargo, productos.descripcion, productos.costo, productos.cod, confirmada FROM ventas, ventas_detalle, productos, usuarios, ventas_forma_pago WHERE ventas.id_venta=ventas_detalle.id_venta AND ventas_detalle.cod_producto=productos.cod AND ventas.id_usuario=usuarios.id_usuario AND ventas.id_usuario='$id_usuario' AND ventas.id_forma=ventas_forma_pago.id_forma AND fecha='".$ult_fecha[0]."' ORDER BY ventas.id_venta DESC",$link);
...
$caja=mysql_query("SELECT caja_extraccion.billetes, caja_extraccion.monedas, caja_extraccion_concepto.nombre, caja_extraccion.fecha, caja_extraccion.hora, caja_extraccion.id_usuario, usuarios.id_usuario, usuarios.usuario, comentarios FROM caja_extraccion, usuarios, caja_extraccion_concepto WHERE concepto=id_concepto AND caja_extraccion.id_usuario=usuarios.id_usuario AND caja_extraccion.id_usuario='$id_usuario' ORDER BY caja_extraccion.fecha DESC",$link);
?>
<html>
<head>
...
<p><strong>REDES SOCIALES</strong><br>
  <a href="http://www.twitter.com/gravitalhouse">TWITTER</a> lokalesspot (lokales181)<br>
  <a href="https://login.secureserver.net/index.php?app=wbe&logout=1">WEBMAIL</a> contacto@lokales.com.ar (park50)<br>
  <a href="http://www.facebook.com/lokales">FACEBOOK</a>mibanez23@hotmail.com (zzzxxx)</p>
...
<table width="95%" border="1" align="center" bordercolor="#999999">
  ...
  <?
  if(mysql_num_rows($ventas)>0)//busca que no haya datos, que sea cero el valor que arroja la consulta
  {
	mysql_data_seek($ventas,0);//nos posiciona en el inicio de la consulta para el nuevo array
  }
while($venta=mysql_fetch_array($ventas))
{
?>
