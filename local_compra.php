<?
	...
	$proveedores=mysql_query("SELECT * FROM proveedores ORDER BY nombre",$link);	
	//$categorias=mysql_query("SELECT * FROM categorias ORDER BY nombre",$link);	
	//if(!isset($_GET['id_cat']))
	//{
	$productos=mysql_query("SELECT productos.cod as cp, productos.nombre as np, costo, descripcion, margen, marcas.nombre as nm FROM productos, categorias, marcas WHERE productos.id_marca=marcas.id_marca AND cod_cat=categorias.cod ORDER BY productos.nombre, marcas.nombre",$link);
	/*}
	else
	{
...
<?
	for($i=0;$i<$cant_registros+1;$i++)
	{
		...
		$temporal=mysql_fetch_array($temporales);
